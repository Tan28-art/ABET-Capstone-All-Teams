...
DESTINATION_COURSE_ID = os.getenv("DESTINATION_COURSE_ID", "240102")
...
def add_to_canvas(course_name, semester, year):
    ...
    try:
        upload_response = requests.post(url=f"{API_BASE_URL}courses/{DESTINATION_COURSE_ID}/pages", json=page_data,
                                        headers=HEADERS)
...
def add_single_module_item(course_id, module_id, page):
    ...
    try:
        ...
        response = requests.post(f"{API_BASE_URL}courses/{course_id}/modules/{module_id}/items", headers=HEADERS,
